...
int main()
{
    ...
    */
    ...
    std::string line;
    std::cout<< "Enter all params: E(MeV) theta_min(degree) theta_max(degree)"<<std::endl;
    ...
    //theta_min = 8;
    //theta_max = 9;
    std::vector<double> numbers( ( std::istream_iterator<int>( iss ) ), std::istream_iterator<int>() );

    for ( double x : numbers) std::cout << x << ' ';
    std::cout << std::endl;
    
    Ei_e = (numbers[0]) * 1e-3;
    theta_min = (numbers[1]) * deg;
    theta_max = (numbers[2]) * deg;
    int N = 1;
    E_g_min = 0.1e-3;
    E_g_cut = 1800.0e-3;
    std::cout<<"check "<<Ei_e<<" "<<theta_min<<" "<<theta_max<<std::endl; 
    ...
    fp = fopen(ifilename, "a");

    fprintf(fp,"Eb_MeV,theta_min_deg,theta_max_deg,E_photon_min_MeV,E_photon_max_MeV,angle_ster,xs_mubperstr,lum_invmub\n");
    fprintf(fp,"%11.5lf,%11.8lf,%11.8lf,%11.8lf,%11.8lf,%11.8lf,%11.8lf,%11.8lf\n",Ei_e*1000,theta_min/deg,theta_max/deg,E_g_min*1000,E_g_cut*1000,omega,xsint/omega,N/xsint);
